<script lang="ts">
	...
	import type { TechniqueQuizQuestion } from '$lib/utils/generateTechniqueQuiz';

	export let data: { questions: TechniqueQuizQuestion[] };

	// ── State ──────────────────────────────────────────────────────────────────
	let questions: TechniqueQuizQuestion[] = [];
	...
	$: resultMessage =
		finalProgress >= 95 ? 'Risultato straordinario! Conosci le tecniche judo alla perfezione!' :
		finalProgress >= 80 ? 'Ottima conoscenza! Continua così!' :
	...
	function getOptions(q: TechniqueQuizQuestion) {
		return [
	...
	function optionState(key: string): 'neutral' | 'correct' | 'wrong' {
	...
	const typeLabel: Record<TechniqueQuizQuestion['type'], string> = {
		terminology: '🈷️ Terminologia',
		gokyo: '📖 Gokyo',
		category: '🥋 Tipologia'
	};
...
<HeaderPages title="Quiz — Tecniche Judo" />
<PageContainer>
		...
		{#if phase === 'intro'}
			<div class="card border border-surface-300-600-token p-8 flex flex-col items-center gap-6 text-center animate-page-in">
				...
				<div class="flex flex-col gap-2">
					<h2 class="font-bold text-2xl">Quiz — Tecniche Judo</h2>
					<p class="opacity-60 text-base">Metti alla prova la tua conoscenza delle tecniche e della terminologia giapponese</p>
				</div>

				{#if data.questions.length === 0}
					<p class="opacity-50 text-sm">Nessuna domanda disponibile.</p>
					<a href="/tecniche" class="btn variant-soft">← Torna alle tecniche</a>
				{:else}
					<div class="flex flex-wrap items-center justify-center gap-4 text-sm opacity-60">
						<span>📝 {data.questions.length} domande</span>
						<span>🈷️ Terminologia JP</span>
						<span>📖 Gokyo</span>
						<span>🥋 Tipologia</span>
					</div>
					...
					<a href="/tecniche" class="text-sm opacity-50 hover:opacity-80 transition-opacity">
						← Torna alle tecniche
					</a>
				{/if}
			</div>
			...
			<div class="flex flex-col gap-4 animate-page-in">
				...
				<div class="card border border-surface-300-600-token p-6 flex flex-col gap-6">
					<div class="flex flex-col gap-3">
						<span class="text-xs opacity-50 font-medium">{typeLabel[current.type]}</span>
		...
		{:else if phase === 'results'}
			...
			>
				...
				<div class="flex flex-wrap gap-3 justify-center mt-2">
					...
					<a href="/tecniche" class="btn variant-soft px-6">
						← Torna alle tecniche
					</a>
				</div>
			</div>
